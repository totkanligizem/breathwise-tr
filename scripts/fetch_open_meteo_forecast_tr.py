...
FORECAST_RAW_DIR = DATA_ROOT / "forecast" / "raw_json"
FORECAST_TIDY_DIR = DATA_ROOT / "forecast" / "tidy"
GEO_DIR = DATA_ROOT / "geocoding"

for folder in [FORECAST_RAW_DIR, FORECAST_TIDY_DIR, GEO_DIR]:
...
# Open-Meteo geocoding'de sorun çıkaran iller için sabit il merkezi koordinatları
CITY_OVERRIDES = {
    "Ağrı": {
        "latitude": 39.7191,
        "longitude": 43.0503,
...
def get_forecast_for_city(session: requests.Session, city: CityMeta) -> dict[str, Any]:
    params = {
    ...
    return safe_request_json(session, FORECAST_URL, params)
...
def parse_daily(city: CityMeta, payload: dict[str, Any]) -> pd.DataFrame:
...
def main() -> None:
    ...
    for idx, city_name in enumerate(PROVINCES_TR, start=1):
        try:
            ...
            payload = get_forecast_for_city(session, city)

            raw_json_path = FORECAST_RAW_DIR / f"{slugify_ascii(city_name)}_forecast_{run_stamp}.json"
            save_json(raw_json_path, payload)

            current_df = parse_current(city, payload)
            hourly_df = parse_hourly(city, payload)
            daily_df = parse_daily(city, payload)
    ...
    if current_frames:
        ...
        all_current.to_csv(
            FORECAST_TIDY_DIR / f"forecast_current_tr_{run_date}.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    if hourly_frames:
        ...
        all_hourly.to_csv(
            FORECAST_TIDY_DIR / f"forecast_hourly_tr_{run_date}.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    if daily_frames:
        ...
        all_daily.to_csv(
            FORECAST_TIDY_DIR / f"forecast_daily_tr_{run_date}.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    if failures:
        pd.DataFrame(failures).to_csv(
            FORECAST_TIDY_DIR / f"forecast_failures_tr_{run_date}.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    print("\nFiles written to:")
    print(f"- {GEO_DIR}")
    print(f"- {FORECAST_RAW_DIR}")
    print(f"- {FORECAST_TIDY_DIR}")
